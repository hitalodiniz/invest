...
export default function DashboardMesa() {
  ...
  // Decisão operacional — combina a lógica da planilha com detecção ITM/OTM:
  //
  // DEFENDER / RECOMPRA  → Put ITM (ação < strike) ou Call ITM (ação > strike)
  //                        independente de % capturado — risco de exercício real
  // MANEJO / AGUARDAR    → campo Exercendo = "Sim" (já em processo de exercício)
  // DADOS INCORRETOS     → prêmio ausente ou zerado
  // AVALIAR ENCERRAMENTO → % lucro capturado ≥ 80% e OTM (seguro encerrar)
  // CARREGAR POSIÇÃO     → demais casos OTM com lucro capturado < 80%
  const obterGatilhoDecisao = (op: any, precoAtualAcao?: number) => {
    ...
    // Detecta ITM com cotação real (se disponível)
    const preco = precoAtualAcao && precoAtualAcao > 0 ? precoAtualAcao : null;
    if (preco && op.strike > 0) {
      const isPut = op.operacao.includes("Put");
      const isCall = op.operacao.includes("Call");
      const itm = (isPut && preco < op.strike) || (isCall && preco > op.strike);
      if (itm)
        return {
    ...
    if (op.exercendo === "Sim")
      return {
        texto: "MANEJO / AGUARDAR VENCIMENTO",
    ...
    if (pct != null && pct >= 80)
      return {
        texto: "AVALIAR ENCERRAMENTO",
        estilo: "bg-emerald-50 text-emerald-700 border-emerald-200",
      };
    return {
      texto: "CARREGAR POSIÇÃO",
      estilo: "bg-sky-50 text-sky-700 border-sky-200",
    };
  };
  ...
  const calcRendimentoTD = (
    ...
    if (!op.valorExercEfetivoTotal || op.valorExercEfetivoTotal <= 0)
      return null;
    const capitalTravado = op.valorExercEfetivoTotal;
    const abertura = parseDataAbertura(op);
    const dataEnc = op.dataEncerramento
      ? new Date(
          op.dataEncerramento.includes("/")
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-x-6 gap-y-4">
                    {[
                      ...
                      ...(op.status === "Zerada"
                        ? [
                            {
                              label: "Rend. TD líquido",
                              val: td ? fmt(td.liquido) : "—",
                              cor: "text-slate-600",
                            },
                            {
                              label: "Líquido da operação",
                              val: liquidoOp != null ? fmt(liquidoOp) : "—",
                              cor:
                                (liquidoOp ?? 0) >= 0
                                  ? "text-emerald-600 font-semibold"
                                  : "text-red-600 font-semibold",
                      ...
                      <div className="bg-white border border-slate-200 rounded-lg p-4 flex flex-col gap-3">
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-x-6 gap-y-3">
                          ...
                          <div>
                            ...
                            <span
                              className={`text-xs font-mono mt-1 block font-semibold ${(liquidoOp ?? 0) >= 0 ? "text-emerald-600" : "text-red-600"}`}
                            >
                          ...
                          <div>
                            ...
                            <span
                              className={`text-xs font-mono mt-1 block font-semibold ${(pctLiquidoOp ?? 0) >= 0 ? "text-emerald-600" : "text-red-600"}`}
                            >
                              {pctLiquidoOp != null
                                ? `${pctLiquidoOp.toFixed(2)}%`
                                : "—"}
                          ...
                          <div>
                            ...
                            <span
                              className={`text-xs font-mono mt-1 block font-semibold ${(premioRetido ?? 0) >= 100 ? "text-emerald-600" : (premioRetido ?? 0) >= 0 ? "text-amber-600" : "text-red-600"}`}
                            >
                              {premioRetido != null
                                ? `${premioRetido.toFixed(2)}%`
                                : "—"}
                  ...
                  {(op.manejo || op.comentario) && (
                    <div className="bg-white border border-slate-200 rounded-lg p-4 flex flex-col gap-2">
                      <div className="flex items-center gap-3 flex-wrap">
                        {op.manejo && (
                          <span
                            className={`text-[11px] px-3 py-1 rounded font-semibold border ${
                              op.manejo === "Pó"
                                ? "bg-slate-100 text-slate-600 border-slate-300"
                                : op.manejo === "DESMONTE"
                                  ? "bg-emerald-50 text-emerald-700 border-emerald-200"
                    ...
                    {editandoEste && (
                      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                        ...
                        <div>
                          ...
                          <select
                            value={editForm.exercendo}
                            onChange={(e) =>
                              setEditForm({
                                ...editForm,
                                exercendo: e.target.value,
                              })
                            }
                            className="w-full bg-white border border-slate-300 rounded-lg px-2 py-1.5 text-xs text-slate-800 focus:outline-none"
                          >
