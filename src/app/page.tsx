...
export default function DashboardMesa() {
  ...
  const [statusUpload, setStatusUpload] = useState<"idle"|"loading"|"success"|"error">("idle");
  ...
  // distanciaStrike pode vir como número (5.23) ou string ("5,23%").
  ...
  // op.data vem no formato DD/MM/YYYY.
  ...
  // Cotação efetiva: manual se editada (cotacaoOpcao ≠ premioUnInicial), senão estimada.
  ...
  const carregarDashboard = () => {
    ...
    fetch("/api/operacoes")
      .then((res) => res.json())
      .then((operacoes) => {
        const ativos = Array.from(new Set(operacoes.map((op: any) => op.ativo)));
        ...
        operacoes.forEach((op: any) => {
          ...
          if (op.status === "Aberta") {
            ...
            if (op.operacao.includes("Put")) {
              ...
              if (op.exercendo === "Sim") desembolsoObrigatorio += op.valorExercEfetivoTotal;
            }
          } else {
        ...
        setData({
          ...
          taxaSucesso: qtdZeradasTotal > 0 ? (qtdZeradasPositivas / qtdZeradasTotal) * 100 : null,
          lucroCapturadoMedio: premioBrutoComCotacao > 0
        ...
        fetch(`/api/market?ativos=${ativos.join(",")}`)
          .then((r) => r.json())
          .then(setMarket)
          .catch((e) => console.error("Erro ao carregar cotações de mercado", e));
      })
      .catch((e) => {
        console.error("Erro ao carregar operações", e);
        setErroCarregamento("Não foi possível carregar as operações. Verifique o console para detalhes e tente novamente.");
      });
  };
  ...
  const handleDeleteOperacao = async (id: string) => {
    ...
    try {
      ...
      if (res.ok) { setIdExpandido(null); carregarDashboard(); }
  ...
  const handleSaveEdicao = async (id: string) => {
    try {
      ...
      if (res.ok) { setIdEditando(null); carregarDashboard(); }
  ...
  const handleUploadNota = async () => {
    ...
    try {
      const res = await fetch("/api/upload-nota", { method: "POST", body: formData });
  ...
  const calcLucroCapturado = (op: any): number | null => {
    ...
    return ((op.premioTotalBruto - cot.valor * op.qtde) / op.premioTotalBruto) * 100;
  };
  ...
  const listaAtivosUnicos = [
    "TODOS",
    ...(Array.from(new Set(data?.operacoes?.map((op: any) => op.ativo))) as string[]),
  ];
  ...
  const operacoesFiltradas = useMemo(() => {
    ...
    const filtradas = (data?.operacoes || []).filter((op: any) => {
      ...
      return (
        ...
        bateVencimento && bateBusca
      );
    });

    return [...filtradas].sort((a: any, b: any) => {
      switch (ordenacao) {
        case "DATA_ANTIGA": return a.data.localeCompare(b.data);
        case "MAIOR_PREMIO": return b.premioTotalBruto - a.premioTotalBruto;
        case "MAIOR_RESULTADO":
          return (b.status === "Zerada" ? b.resultadoLiquido : b.resultadoBrutoReal) -
            (a.status === "Zerada" ? a.resultadoLiquido : a.resultadoBrutoReal);
        case "MENOR_RESULTADO":
          return (a.status === "Zerada" ? a.resultadoLiquido : a.resultadoBrutoReal) -
            (b.status === "Zerada" ? b.resultadoLiquido : b.resultadoBrutoReal);
        default: return b.data.localeCompare(a.data);
      }
    });
  }, [data, market, filtroAtivo, filtroTipo, filtroDecisao, filtroStatus, abaVencimento, busca, ordenacao]);
  ...
  return (
    <main className="min-h-screen bg-slate-50 text-slate-700 px-4 py-6 md:px-8 flex flex-col gap-5 w-full max-w-[100vw] overflow-x-hidden antialiased">
      ...
      <div className="flex justify-between items-start gap-3 flex-wrap border-b border-slate-200 pb-5">
        <div>
          <h1 className="text-base font-semibold text-slate-900 tracking-tight uppercase">Mesa Automatizada de Renda</h1>
          <p className="text-[11px] text-slate-400 uppercase tracking-widest mt-1">Toque em uma posição para ver todos os detalhes</p>
        </div>
        <div className="flex gap-2 flex-wrap">
          <button onClick={() => { carregarNotas(); setModalNotasAberto(true); }}
          ...
          <button onClick={() => setModalUploadAberto(true)}
      ...
      <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <div className="flex items-baseline gap-3 mt-1">
            <span className="text-4xl font-semibold font-mono text-emerald-700 leading-none">
              {data?.lucroCapturadoMedio != null ? `${(data.lucroCapturadoMedio as number).toFixed(1)}%` : "—"}
            </span>
            <span className="text-sm text-emerald-600">do prêmio inicial já garantido</span>
          </div>
          <p className="text-xs text-emerald-600 mt-1.5 opacity-80">
            Quanto de cada prêmio recebido já não precisa ser devolvido ao mercado
          </p>
          <p className="text-xs font-mono font-semibold text-emerald-700 mt-1">
            {fmt(data?.resultadoNaoRealizadoTotal ?? 0)} em lucro não realizado (bruto, antes de custos de recompra e IR)
          </p>
        </div>
        <div className="flex flex-col items-start sm:items-end gap-2 shrink-0">
          ...
          <span className="text-xs text-emerald-700">
            {data?.qtdAbertas ?? 0} abertas · {(data?.qtdTotal ?? 0) - (data?.qtdAbertas ?? 0)} zeradas
          </span>
        </div>
      </div>
      ...
      <div className="flex gap-0.5 border-b border-slate-200 overflow-x-auto -mx-4 px-4 md:mx-0 md:px-0">
        {["TODOS", ...abasVencimento].map((chave) => (
          <button key={chave} onClick={() => setAbaVencimento(chave)}
            className={`px-4 py-2 text-[11px] font-semibold uppercase tracking-widest border-b-2 transition-colors whitespace-nowrap shrink-0 ${
              abaVencimento === chave ? "border-slate-900 text-slate-900" : "border-transparent text-slate-400 hover:text-slate-600"
            }`}>
      ...
      {/* ── Capital ── */}
      <div>
        <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-widest mb-2">Alocação de capital</p>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          <div className="bg-white border border-slate-200 rounded-xl p-4">
            <p className="text-[11px] text-slate-500 font-semibold uppercase tracking-wider">Capital de garantia</p>
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-4">
            <p className="text-[11px] text-slate-500 font-semibold uppercase tracking-wider">Capital travado puts</p>
            <p className="text-sm font-mono font-semibold text-amber-700 mt-1.5">{fmt(data?.capitalTravadoPuts ?? 0)}</p>
          </div>
          <div className="bg-white border border-slate-200 rounded-xl p-4">
            <p className="text-[11px] text-slate-500 font-semibold uppercase tracking-wider">Desembolso obrigatório</p>
            <p className="text-sm font-mono font-semibold text-red-700 mt-1.5">{fmt(data?.desembolsoObrigatorio ?? 0)}</p>
          </div>
          <div className={`rounded-xl p-4 border ${margemDisponivel >= 0 ? "bg-emerald-50 border-emerald-200" : "bg-red-50 border-red-200"}`}>
            <p className={`text-[11px] font-semibold uppercase tracking-wider ${margemDisponivel >= 0 ? "text-emerald-700" : "text-red-700"}`}>Margem disponível</p>
            <p className={`text-sm font-mono font-semibold mt-1.5 ${margemDisponivel >= 0 ? "text-emerald-700" : "text-red-700"}`}>{fmt(margemDisponivel)}</p>
          </div>
        </div>
      </div>
      ...
      {/* ── KPIs de performance ── */}
      <div>
        <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-widest mb-2">Performance</p>
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
          {[
            { label: "Prêmio recebido (abertas)", val: fmt(data?.premioBruto ?? 0), cor: "text-sky-700" },
            { label: "P&L não-realizado", val: fmt(data?.resultadoNaoRealizadoTotal ?? 0), cor: (data?.resultadoNaoRealizadoTotal ?? 0) >= 0 ? "text-emerald-700" : "text-red-700" },
            { label: "P&L realizado", val: fmt(data?.resultadoRealizadoTotal ?? 0), cor: (data?.resultadoRealizadoTotal ?? 0) >= 0 ? "text-emerald-700" : "text-red-700" },
            { label: "Provisão DARF", val: fmt(data?.provisaoDarfTotal ?? 0), cor: "text-red-700" },
            { label: "Taxa de sucesso", val: data?.taxaSucesso != null ? `${(data.taxaSucesso as number).toFixed(1)}%` : "—", cor: "text-slate-800" },
            { label: "Posições abertas", val: `${data?.qtdAbertas ?? 0} / ${data?.qtdTotal ?? 0}`, cor: "text-slate-800" },
          ].map((k) => (
            <div key={k.label} className="bg-white border border-slate-200 rounded-xl p-4">
              <p className="text-[11px] text-slate-500 font-semibold uppercase tracking-wider leading-tight">{k.label}</p>
              <p className={`text-sm font-mono font-semibold mt-1.5 ${k.cor}`}>{k.val}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-4 flex flex-col gap-3">
        <div className="flex flex-wrap gap-2">
          ...
          <select value={filtroAtivo} onChange={(e) => setFiltroAtivo(e.target.value)}
            ...
            {listaAtivosUnicos.map((at) => <option key={at} value={at}>{at === "TODOS" ? "Todos os ativos" : at}</option>)}
          </select>
        ...
        <div className="flex flex-wrap gap-2 items-center">
          ...
          <span className="text-[11px] text-slate-400 font-mono ml-auto">
            {operacoesFiltradas.length} de {data?.operacoes?.length ?? 0} operações
          </span>
        </div>
      </div>
          ...
          return (
            <div key={op.id}
              className={`bg-white border rounded-xl overflow-hidden transition-all duration-100 ${
                expandido ? "border-slate-400 shadow-md" : "border-slate-200 hover:border-slate-300"
              }`}>
              {/* Capa */}
              <div onClick={() => setIdExpandido(expandido ? null : op.id)}
                className="p-4 md:py-4 md:px-6 flex flex-col gap-3 cursor-pointer select-none">

                {/* Linha 1: ativo + código + badges */}
                <div className="flex items-start justify-between gap-3">
                  <div className="flex items-center gap-4">
                    <div>
                      <span className="text-[10px] text-slate-400 font-mono block">{op.data}</span>
                      <span className="text-base font-semibold text-slate-900 block">{op.ativo}</span>
                    </div>
                    <div>
                      <span className="text-[10px] text-slate-400 uppercase tracking-wider block">Código</span>
                      <div className="flex items-center gap-1.5 mt-0.5">
                        <span className="text-sm font-semibold text-amber-700 font-mono">{op.codigo}</span>
                        <span className="text-[10px] text-slate-400 font-mono">({op.tipo})</span>
                      </div>
                    </div>
                  </div>
                  <div className="flex flex-col items-end gap-1.5 shrink-0">
                    {op.precisaRevisao && (
                      <span className="text-[10px] px-2.5 py-1 rounded font-semibold border bg-orange-50 text-orange-700 border-orange-300">⚠️ REVISAR</span>
                    )}
                    {op.manejo && op.status === "Zerada" && (
                      <span className={`text-[10px] px-2.5 py-1 rounded font-semibold border ${
                        op.manejo === "Pó" ? "bg-slate-100 text-slate-600 border-slate-300" :
                        op.manejo === "DESMONTE" ? "bg-emerald-50 text-emerald-700 border-emerald-200" :
                        op.manejo === "Rolagem" ? "bg-sky-50 text-sky-700 border-sky-200" :
                        op.manejo === "Exercido" ? "bg-red-50 text-red-700 border-red-200" :
                        "bg-slate-100 text-slate-500 border-slate-200"
                      }`}>{op.manejo}</span>
                    )}
                    <span className={`text-[10px] px-2.5 py-1 rounded font-semibold border ${gatilho.estilo}`}>{gatilho.texto}</span>
                    <span className="text-[10px] text-slate-400">{expandido ? "▲ fechar" : "▼ detalhes"}</span>
                  </div>
                </div>

                {/* Linha 2: grid de métricas de decisão */}
                <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-x-4 gap-y-2.5">
                  {/* Cotação ação */}
                  <div>
                    <span className="text-[10px] text-slate-400 uppercase tracking-wider flex items-center gap-1">
                      Cotação ação
                      <span title={emTempoReal ? "Cotação obtida em tempo real" : "Cotação estática (última nota / sem API disponível)"}
                        className={`w-1.5 h-1.5 rounded-full inline-block ${emTempoReal ? "bg-emerald-500" : "bg-amber-400"}`} />
                    </span>
                    <span className="text-sm font-mono font-semibold text-sky-600 mt-0.5 block">{fmt(precoAcaoReal)}</span>
                    <span className={`text-[9px] font-semibold uppercase tracking-wider ${emTempoReal ? "text-emerald-600" : "text-amber-600"}`}>
                      {emTempoReal ? "tempo real" : "estática · última nota"}
                    </span>
                  </div>
                  {/* Qtde */}
                  <div>
                    <span className="text-[10px] text-slate-400 uppercase tracking-wider">Qtde (contratos)</span>
                    <span className="text-sm font-mono font-semibold text-slate-700 mt-0.5 block">{op.qtde?.toLocaleString("pt-BR")}</span>
                  </div>
                  {/* Strike */}
                  <div>
                    <span className="text-[10px] text-slate-400 uppercase tracking-wider">Strike</span>
                    <span className="text-sm font-mono text-slate-700 mt-0.5 block">
                      {fmt(op.strike)}
                      <span className={`text-xs ml-1 font-semibold ${distNum < 0 ? "text-emerald-600" : "text-red-600"}`}>
                  ...
                  {/* Prêmio */}
                  <div>
                    <span className="text-[10px] text-slate-400 uppercase tracking-wider">Prêmio recebido</span>
                    <span className="text-sm font-mono font-semibold text-emerald-600 mt-0.5 block">{fmt(op.premioTotalBruto)}</span>
                  </div>
                  {/* % Lucro capturado */}
                  <div>
                    <span className="text-[10px] text-slate-400 uppercase tracking-wider">% Lucro capturado</span>
                    <div className="mt-0.5 flex flex-col gap-0.5">
                      {op.status === "Aberta" ? (
                        pctLucro != null ? (
                          <>
                            <span className={`inline-flex w-fit items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold border ${
                              pctLucro >= 0 ? "bg-emerald-50 text-emerald-700 border-emerald-200" : "bg-red-50 text-red-700 border-red-200"
                            }`}>
                              {pctLucro >= 0 ? "↑" : "↓"} {Math.abs(pctLucro).toFixed(1)}%
                            </span>
                            <span className={`text-[10px] font-mono font-semibold ${(resultadoNaoRealizado ?? 0) >= 0 ? "text-emerald-600" : "text-red-600"}`}>
                              {fmt(resultadoNaoRealizado ?? 0)}
                            </span>
                            {cotEfet?.origem === "estimada" && (
                              <span className="text-[9px] text-slate-400 italic">estimado · decaimento</span>
                            )}
                          </>
                  ...
                  {op.status === "Aberta" && (
                    <div>
                      <span className="text-[10px] text-slate-400 uppercase tracking-wider">Custo recompra</span>
                      <span className="text-sm font-mono font-semibold text-red-600 mt-0.5 block">
                        {custoRecompraTotal != null ? fmt(custoRecompraTotal) : "—"}
                      </span>
                      {cotEfet && (
                        <span className="text-[9px] text-slate-400">
                          @ {cotEfet.valor.toFixed(2)}/un {cotEfet.origem === "manual" ? "· manual" : "· estimado"}
                        </span>
                      )}
                    </div>
                  )}
                  {/* Vencimento */}
                  <div>
                    <span className="text-[10px] text-slate-400 uppercase tracking-wider">Vencimento</span>
                    {op.vencimento ? (
                      <>
                        <span className="text-sm font-mono text-slate-700 mt-0.5 block">
                          {new Date(op.vencimento + "T00:00:00").toLocaleDateString("pt-BR")}
                        </span>
                        {diasParaVenc != null && op.status === "Aberta" && (
                          <span className={`text-[10px] font-semibold ${
                            diasParaVenc <= 5 ? "text-red-600" : diasParaVenc <= 14 ? "text-amber-600" : "text-slate-400"
                          }`}>
                            {diasParaVenc > 0 ? `${diasParaVenc}d restantes` : diasParaVenc === 0 ? "vence hoje" : "vencido"}
                          </span>
                        )}
                      </>
              ...
              {expandido && (
                <div className="border-t border-slate-100 bg-slate-50 px-4 md:px-6 py-5 flex flex-col gap-4">
                  ...
                  {op.status === "Aberta" && pctLucro != null && (
                    <div className={`rounded-lg p-4 flex items-center justify-between gap-4 flex-wrap border ${
                      pctLucro >= 0 ? "bg-emerald-50 border-emerald-200" : "bg-red-50 border-red-200"
                    }`}>
                      <div>
                        <p className={`text-[11px] font-semibold uppercase tracking-widest ${pctLucro >= 0 ? "text-emerald-700" : "text-red-700"}`}>
                          % Lucro capturado
                        </p>
                        <p className={`text-2xl font-mono font-semibold mt-0.5 ${pctLucro >= 0 ? "text-emerald-700" : "text-red-700"}`}>
                          {pctLucro.toFixed(1)}%
                        </p>
                      </div>
                      <div className="text-right">
                        <p className={`text-xs ${pctLucro >= 0 ? "text-emerald-600" : "text-red-600"}`}>
                          {fmt(resultadoNaoRealizado ?? 0)} de {fmt(op.premioTotalBruto)} garantidos
                        </p>
                        {custoRecompraTotal != null && (
                          <p className="text-xs text-slate-500 mt-0.5">Custo de recompra atual: {fmt(custoRecompraTotal)}</p>
                        )}
                        <p className="text-[10px] text-slate-400 mt-1">
                          Valor bruto — não desconta emolumentos/corretagem de recompra nem eventual DARF
                        </p>
                      </div>
                    </div>
                  )}
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-x-6 gap-y-4">
                    {[
                      ...
                      { label: "Código", val: op.codigo, cor: "text-amber-700 font-semibold" },
                      ...
                      { label: "Cotação opção (manual)", val: op.cotacaoOpcao ? fmt(op.cotacaoOpcao) : "—", cor: "text-violet-600" },
                      { label: "% Lucro capturado", val: pctLucro != null ? `${pctLucro.toFixed(1)}%` : "—", cor: pctLucro != null ? (pctLucro >= 0 ? "text-emerald-600 font-semibold" : "text-red-600 font-semibold") : "" },
                      { label: "Lucro capturado (R$, bruto)", val: resultadoNaoRealizado != null ? fmt(resultadoNaoRealizado) : "—", cor: resultadoNaoRealizado != null ? (resultadoNaoRealizado >= 0 ? "text-emerald-600 font-semibold" : "text-red-600 font-semibold") : "" },
                      { label: "Custo recompra total", val: custoRecompraTotal != null ? fmt(custoRecompraTotal) : "—", cor: "text-red-600" },
                      { label: "Resultado bruto real", val: fmt(op.resultadoBrutoReal), cor: (op.resultadoBrutoReal ?? 0) >= 0 ? "text-emerald-600" : "text-red-600" },
                      { label: "Valor exercício un.", val: fmt(op.strike), cor: "" },
                      ...
                      <div key={label}>
                        ...
                        <span className={`text-xs font-mono mt-1 block ${cor || "text-slate-700"}`}>{val ?? "—"}</span>
                      </div>
                    ))}
                    <div className="col-span-2 sm:col-span-3 md:col-span-4">
                      <span className="text-[10px] text-slate-400 font-semibold uppercase tracking-wider block">Decisão operacional</span>
                      ...
                      <div className="flex items-center gap-3 flex-wrap">
                        {op.manejo && (
                          <span className={`text-[11px] px-3 py-1 rounded font-semibold border ${
                            ...
                            {op.manejo === "Pó" ? "💨 Pó" :
                             op.manejo === "DESMONTE" ? "✓ Desmonte" :
                             op.manejo === "Rolagem" ? "🔄 Rolagem" :
                             op.manejo === "Exercido" ? "⚡ Exercido" : op.manejo}
                          </span>
                        )}
                        {op.comentario && (
                          <p className="text-xs text-slate-500 italic">{op.comentario}</p>
                        )}
                      </div>
                    </div>
                  )}
                  ...
                  <div className="bg-white border border-slate-200 rounded-lg p-4 flex flex-col gap-3">
                    <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-widest">Custos e rastreabilidade</p>
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-x-6 gap-y-3">
                      <div>
                        <span className="text-[10px] text-slate-400 font-semibold uppercase tracking-wider block">Custos rateados</span>
                        <span className="text-xs font-mono mt-1 block text-red-600">{op.custosRateados != null ? fmt(op.custosRateados) : "—"}</span>
                      </div>
                      <div>
                        <span className="text-[10px] text-slate-400 font-semibold uppercase tracking-wider block">Nota de abertura</span>
                        <span className="text-xs font-mono mt-1 block text-slate-700">{op.numeroNotaAbertura ?? "—"}</span>
                      </div>
                      <div>
                        <span className="text-[10px] text-slate-400 font-semibold uppercase tracking-wider block">Nota de fechamento</span>
                        <span className="text-xs font-mono mt-1 block text-slate-700">{op.numeroNotaFechamento ?? "—"}</span>
                      </div>
                      <div>
                        <span className="text-[10px] text-slate-400 font-semibold uppercase tracking-wider block">Situação</span>
                        {op.precisaRevisao ? (
                          <span className="inline-block mt-1 text-[11px] px-2.5 py-0.5 rounded font-semibold border bg-orange-50 text-orange-700 border-orange-300">⚠️ Recompra sem posição — revisar</span>
                        ) : (
                          <span className="inline-block mt-1 text-[11px] px-2.5 py-0.5 rounded font-semibold border bg-emerald-50 text-emerald-700 border-emerald-200">✓ OK</span>
                        )}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col gap-3 border-t border-slate-200 pt-3">
                    {editandoEste && (
                      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                        {[
                          { label: "Qtde", key: "qtde", type: "number", step: "1" },
                          { label: "Strike", key: "strike", type: "number", step: "0.01" },
                          { label: "Cotação opção", key: "cotacaoOpcao", type: "number", step: "0.01" },
                          ...
                          <div key={key}>
                            ...
                            <input type={type} step={step} value={editForm[key]}
                              onChange={(e) => setEditForm({ ...editForm, [key]: e.target.value !== "" ? parseFloat(e.target.value) : "" })}
                        ...
                        <div>
                          <label className="text-[10px] text-slate-400 uppercase tracking-wider block mb-1">Exercendo?</label>
                          <select value={editForm.exercendo} onChange={(e) => setEditForm({ ...editForm, exercendo: e.target.value })}
                            className="w-full bg-white border border-slate-300 rounded-lg px-2 py-1.5 text-xs text-slate-800 focus:outline-none">
                        ...
                        <div>
                          <label className="text-[10px] text-slate-400 uppercase tracking-wider block mb-1">Vencimento</label>
                          <input type="date" value={editForm.vencimento}
                            onChange={(e) => setEditForm({ ...editForm, vencimento: e.target.value })}
                            className="w-full bg-white border border-slate-300 rounded-lg px-2 py-1.5 text-xs text-slate-800 focus:outline-none" />
                        </div>
                        <div>
                          <label className="text-[10px] text-slate-400 uppercase tracking-wider block mb-1">Manejo</label>
                          <select value={editForm.manejo} onChange={(e) => setEditForm({ ...editForm, manejo: e.target.value })}
                            className="w-full bg-white border border-slate-300 rounded-lg px-2 py-1.5 text-xs text-slate-800 focus:outline-none">
                        ...
                        <div className="col-span-2 sm:col-span-3">
                          <label className="text-[10px] text-slate-400 uppercase tracking-wider block mb-1">Comentário</label>
                          <input type="text" value={editForm.comentario}
                    ...
                    <div className="flex justify-end gap-2">
                      {editandoEste ? (
                        ...
                        <>
                          ...
                          <button onClick={() => handleDeleteOperacao(op.id)}
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4">
          <div className="bg-white border border-slate-200 rounded-xl shadow-2xl w-full max-w-3xl max-h-[90vh] flex flex-col">
            <div className="flex justify-between items-center px-6 py-4 border-b border-slate-100 shrink-0">
              <h3 className="text-sm font-semibold text-slate-900 uppercase tracking-wider">🧾 Notas processadas ({notas.length})</h3>
              <button onClick={() => { setModalNotasAberto(false); setNotaSelecionada(null); }}
            ...
            <div className="flex flex-col sm:flex-row flex-1 overflow-hidden min-h-0">
              <div className="sm:w-56 border-b sm:border-b-0 sm:border-r border-slate-100 overflow-y-auto shrink-0">
                {notas.length === 0 ? (
                  <p className="text-xs text-slate-400 text-center py-8">Nenhuma nota importada.</p>
                ) : (
                  <ul className="divide-y divide-slate-100">
                    {notas.map((nota: any) => (
                      <li key={nota.id}>
                        <button onClick={() => setNotaSelecionada(nota)}
                          className={`w-full text-left px-4 py-3 hover:bg-slate-50 transition-colors ${notaSelecionada?.id === nota.id ? "bg-slate-100" : ""}`}>
                          <span className="block text-xs font-semibold text-slate-900 font-mono">#{nota.id}</span>
                          <span className="block text-[10px] text-slate-400 mt-0.5">{nota.dataPregao ?? nota.data_importacao}</span>
                          <span className="block text-[10px] text-slate-500 mt-0.5">{nota.corretora ?? "—"}</span>
                          <span className={`block text-xs font-mono font-semibold mt-1 ${(nota.liquidoNota ?? 0) >= 0 ? "text-emerald-600" : "text-red-600"}`}>
              ...
              <div className="flex-1 overflow-y-auto p-5">
                {!notaSelecionada ? (
                  <p className="text-xs text-slate-400 text-center mt-8">Selecione uma nota para ver o detalhamento de taxas.</p>
                ) : (
                  <div className="flex flex-col gap-4">
                    <div>
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        {[
                          { label: "Número da nota", val: `#${notaSelecionada.id}` },
                          { label: "Pregão", val: notaSelecionada.dataPregao ?? "—" },
                          { label: "Corretora", val: notaSelecionada.corretora ?? "—" },
                          { label: "Importada em", val: notaSelecionada.data_importacao },
                        ].map(({ label, val }) => (
                          <div key={label}>
                            <span className="text-[10px] text-slate-400 font-semibold uppercase tracking-wider block">{label}</span>
                            <span className="text-xs font-mono mt-0.5 block text-slate-700">{val}</span>
                          </div>
                        ))}
                      </div>
                    </div>
                    <div>
                      <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-widest mb-3">Resumo operacional</p>
                      <div className="grid grid-cols-2 gap-3">
                        <div>
                          <span className="text-[10px] text-slate-400 font-semibold uppercase tracking-wider block">Valor líq. operações</span>
                          <span className="text-xs font-mono mt-0.5 block text-slate-800 font-semibold">{fmt(notaSelecionada.valorLiquidoOperacoes ?? 0)}</span>
                        </div>
                        <div>
                          <span className="text-[10px] text-slate-400 font-semibold uppercase tracking-wider block">Líquido da nota</span>
                          <span className={`text-xs font-mono mt-0.5 block font-semibold ${(notaSelecionada.liquidoNota ?? 0) >= 0 ? "text-emerald-600" : "text-red-600"}`}>
                    ...
                    <div>
                      <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-widest mb-3">Taxas CBLC</p>
                      <div className="grid grid-cols-2 gap-3">
                        {[
                          { label: "Taxa de liquidação", val: notaSelecionada.taxaLiquidacao },
                          { label: "Taxa de registro", val: notaSelecionada.taxaRegistro },
                          { label: "Total CBLC", val: notaSelecionada.totalCBLC, destaque: true },
                        ].map(({ label, val, destaque }: any) => (
                          <div key={label}>
                            <span className="text-[10px] text-slate-400 font-semibold uppercase tracking-wider block">{label}</span>
                            <span className={`text-xs font-mono mt-0.5 block ${destaque ? "font-semibold text-red-600" : "text-slate-700"}`}>{fmt(val ?? 0)}</span>
                          </div>
                        ))}
                      </div>
                    </div>
                    <div>
                      <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-widest mb-3">Taxas B3 / Bovespa</p>
                      <div className="grid grid-cols-2 gap-3">
                        {[
                          { label: "Taxa Termo/Opções", val: notaSelecionada.taxaTermoOpcoes },
                          { label: "Taxa ANA", val: notaSelecionada.taxaANA },
                          { label: "Emolumentos", val: notaSelecionada.emolumentos },
                          { label: "Taxa transf. ativos", val: notaSelecionada.taxaTransfAtivos },
                          { label: "Total B3 (soma)", val: notaSelecionada.totalBovespaSoma, destaque: true },
                        ].map(({ label, val, destaque }: any) => (
                          <div key={label}>
                            <span className="text-[10px] text-slate-400 font-semibold uppercase tracking-wider block">{label}</span>
                            <span className={`text-xs font-mono mt-0.5 block ${destaque ? "font-semibold text-red-600" : "text-slate-700"}`}>{fmt(val ?? 0)}</span>
                    ...
                    <div>
                      <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-widest mb-3">Custos operacionais e IR</p>
                      <div className="grid grid-cols-2 gap-3">
                        {[
                          { label: "Taxa operacional", val: notaSelecionada.taxaOperacional },
                          { label: "Execução", val: notaSelecionada.execucao },
                          { label: "Taxa de custódia", val: notaSelecionada.taxaCustodia },
                          { label: "Impostos", val: notaSelecionada.impostos },
                          { label: "IRRF retido", val: notaSelecionada.irrf, cor: "text-amber-700 font-semibold" },
                          { label: "Outros", val: notaSelecionada.outros },
                          { label: "Total custos e despesas", val: notaSelecionada.totalCustosDespesas, destaque: true },
                        ].map(({ label, val, destaque, cor }: any) => (
                          <div key={label}>
                            <span className="text-[10px] text-slate-400 font-semibold uppercase tracking-wider block">{label}</span>
                            <span className={`text-xs font-mono mt-0.5 block ${cor ?? (destaque ? "font-semibold text-red-600" : "text-slate-700")}`}>{fmt(val ?? 0)}</span>
                          </div>
                        ))}
                      </div>
                    </div>
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4">
          <div className="bg-white border border-slate-200 rounded-xl shadow-2xl p-6 w-full max-w-md">
            <div className="flex justify-between items-center mb-5 border-b border-slate-100 pb-3">
              <h3 className="text-sm font-semibold text-slate-900 uppercase tracking-wider">Importação de nota Clear</h3>
              <button onClick={() => setModalUploadAberto(false)} className="text-slate-400 hover:text-slate-700 text-xl leading-none">×</button>
            </div>
            <div className="flex flex-col gap-4">
              <div>
                <label className="text-[11px] text-slate-500 font-semibold uppercase tracking-wider block mb-1.5">1. Selecione o arquivo PDF</label>
                <input type="file" accept=".pdf" ref={fileInputRef}
                  onChange={(e) => setArquivoNota(e.target.files ? e.target.files[0] : null)}
              ...
              <div>
                <label className="text-[11px] text-slate-500 font-semibold uppercase tracking-wider block mb-1.5">2. Senha do arquivo</label>
                <input type="password" value={senhaNota} onChange={(e) => setSenhaNota(e.target.value)}
              ...
              {statusUpload === "loading" && <p className="text-xs text-amber-600 font-mono text-center animate-pulse">Processando nota…</p>}
              {statusUpload === "success" && <p className="text-xs text-emerald-600 font-mono text-center font-semibold">✅ Nota gravada com sucesso!</p>}
              {statusUpload === "error" && <p className="text-xs text-red-600 font-mono text-center font-semibold">❌ {mensagemErro || "Falha na leitura."}</p>}
              <button onClick={handleUploadNota} disabled={!arquivoNota || statusUpload === "loading"}
